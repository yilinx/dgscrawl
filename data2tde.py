...
# <br\>
# In this function, the merged data is converted into a Tableau Data Extract and zipped up. This is because tde files will be blocked by government's mailbox.
...
for item in cheader:
    if item.find('last_updated') >= 0:
        dataSchema.addColumn(item,tde.Type.DATETIME)
    elif item == 'sn':
        dataSchema.addColumn(item,tde.Type.DOUBLE)
    else:
        dataSchema.addColumn(item,tde.Type.CHAR_STRING)
...
for i in range(0, len(dataset)):
    for nitem in range(0,len(cheader)):
        if cheader[nitem].find('last_updated') >= 0:
            ...
            newRow.setDouble(nitem,dataset[cheader[nitem]][i])
        else:
            try:
